...
# ── Clean Encoder (Lower Delta Threshold = 0.02) ──
def encode_audio_clean(audio):
    ...
    # Delta modulation -> THRESHOLD = 0.02
    delta = np.abs(np.diff(log_mel, axis=1, prepend=log_mel[:, :1]))
    binary = (delta > 0.02).astype(np.float32)
...
def test_overfit_batch():
    ...
    print("Trying to overfit 1 batch (size 32) with THRESHOLD=0.02...")
