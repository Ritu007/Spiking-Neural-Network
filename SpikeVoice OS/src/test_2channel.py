...
def encode_audio_2channel(audio):
    mel = librosa.feature.melspectrogram(
        y=audio, sr=16000, n_fft=1024, hop_length=640, win_length=640, n_mels=128, fmin=20, fmax=8000
    )
    log_mel = librosa.power_to_db(mel, ref=np.max)
    log_mel = np.clip((log_mel + 80.0) / 80.0, 0.0, 1.0)
    
    diff = np.diff(log_mel, axis=1, prepend=log_mel[:, :1])
    # Lowered threshold to 0.05 to capture more nuanced speech dynamics
    on_spikes = (diff > 0.05).astype(np.float32)
    off_spikes = (diff < -0.05).astype(np.float32)
    
    binary = np.stack([on_spikes, off_spikes], axis=0) # [2, 128, frames]
    
    # Pad or crop to exactly 25
    if binary.shape[2] < 25:
        binary = np.pad(binary, ((0, 0), (0, 0), (0, 25 - binary.shape[2])))
    else:
        binary = binary[:, :, :25]
        
    # Shape: [2, 128, 25] -> [25, 2, 128]
    binary = binary.transpose(2, 0, 1)
    spikes = torch.from_numpy(binary)
    return spikes

class TwoChannelDataset(KeywordDataset):
    def __getitem__(self, idx):
        waveform, sr, label, *_ = self.dataset[self.indices[idx]]
        audio = waveform.squeeze(0).numpy()
        spikes = encode_audio_2channel(audio)
...
class TwoChannelModel(nn.Module):
    def __init__(self, beta=0.9, threshold=0.8):
        ...
        self.conv1 = nn.Conv1d(2, 32, kernel_size=3, padding=1, bias=False)
...
def test_overfit_batch():
    ...
    dataset = TwoChannelDataset(subset="training")
    loader = torch.utils.data.DataLoader(dataset, batch_size=32, shuffle=True)
    
    spikes, labels = next(iter(loader))
    # Note: DataLoader adds batch dim at pos 0, shape is [B, T, 2, 128] -> [T, B, 2, 128]
    spikes = spikes.permute(1, 0, 2, 3).to(device, dtype=torch.float32)
    labels = labels.to(device)
    
    model = TwoChannelModel().to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    
    print("Trying to overfit 1 batch (size 32) with 2-Channel ON/OFF Delta Encoder...")
