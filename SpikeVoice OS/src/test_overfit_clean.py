...
# ── Clean Encoder (T=25 naturally via 40ms hops) ──
def encode_audio_clean(audio):
    # 40ms hop -> exactly 26 frames for 1s audio
...
class CleanDataset(KeywordDataset):
    def __getitem__(self, idx):
        waveform, sr, label, *_ = self.dataset[self.indices[idx]]
        audio = waveform.squeeze(0).numpy()
        spikes = encode_audio_clean(audio)
        class_id = self.LABEL_MAP.get(label, 5)
        return spikes, class_id

# ── Clean Model (No Temporal Dropout, Summed Logits, GroupNorm) ──
...
def test_overfit_batch():
    ...
    dataset = CleanDataset(subset="training")
    ...
    model = CleanModel().to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    
    print("Trying to overfit 1 batch (size 32) with Natural T=25, NO dropout, Summed Logits...")
